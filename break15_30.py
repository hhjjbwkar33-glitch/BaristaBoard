    ...
    for slot, names in schedule.items():
        if len(names) >= 3:
            ...
            if fs_start >= start_time + timedelta(hours=2) and fs_start < end_time - timedelta(hours=2):
                break_30x2slot.append((fs_start, fs_end))
    if not break_30x2slot:
        return None
    half = len(break_30x2slot) // 2
    quarter = half // 2
    start_break1, end_break1 = break_30x2slot[quarter]
    start_break2, end_break2 = break_30x2slot[half + quarter]
...
def generate_break(start, end, schedule):
    ...
    if work_time < timedelta(hours=5):
        ...
        for slot, names in schedule.items():
            if len(names) >= 3:
                fs15_start, fs15_end = slot.split('-')
                fs15_start = datetime.strptime(fs15_start, '%H:%M')
                fs15_end = datetime.strptime(fs15_end, '%H:%M')
                if fs15_start > start_time + timedelta(hours=2) and fs15_start < end_time - timedelta(hours=2):
                    break_15slot.append((fs15_start, fs15_end))
        if not break_15slot:
            return None
        half = len(break_15slot) // 2
        start_15break, end_15break = break_15slot[half]
        return(start_15break, start_15break + timedelta(minutes=15), 15)
    elif work_time < timedelta(hours=7):
        for slot, names in schedule.items():
            if len(names) >= 3:
                fs30_start, fs30_end = slot.split('-')
                fs30_start = datetime.strptime(fs30_start, '%H:%M')
                fs30_end = datetime.strptime(fs30_end, '%H:%M')
                if fs30_start > start_time + timedelta(hours=2) and fs30_start < end_time - timedelta(hours=2):
                    break_30slot.append((fs30_start, fs30_end))
        if not break_30slot:
            return None
        half = len(break_30slot) // 2
        start_30break, end_30break = break_30slot[half]
        return(start_30break, end_30break, 30)
    elif work_time < timedelta(hours=8):
        for slot, names in schedule.items():
            if len(names) >= 3:
                fs45_start, fs45_end = slot.split('-')
                fs45_start = datetime.strptime(fs45_start, '%H:%M')
                fs45_end = datetime.strptime(fs45_end, '%H:%M')
                if fs45_start > start_time + timedelta(hours=2) and fs45_start < end_time - timedelta(hours=2):
                    break_45slot.append((fs45_start, fs45_end))
        if not break_45slot:
            return None
        half = len(break_45slot) // 2
        start_45break, end_45break = break_45slot[half]
        return[
            (start_45break, start_45break + timedelta(minutes=30), 30),
            (start_45break + timedelta(minutes=30), start_45break + timedelta(minutes=45), 15)
